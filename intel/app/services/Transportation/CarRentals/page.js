...
const CarRentals = () => {
  ...
  const fetchCarRentals = async (userCoordinates) => {
    ...
    try {
      ...
      const backendResponse = await axios.get(`/api/transportation/carRental?lon=${userCoordinates.lon}&lat=${userCoordinates.lat}&domain=Transportation&serviceType=Car Rentals`);
    ...
    <div className="min-h-screen bg-gray-100">
      <main className="container mx-auto px-4 py-8">
        <section className="mb-12">
          <h2 className="text-2xl sm:text-3xl font-bold mb-6 text-center text-blue-600">Search for Car Rentals Near You</h2>
              ...
              <section className="mb-12">
                <div className="space-y-8">
                  {carRentals.map((rental) => (
                    <div key={rental.id} className="bg-white shadow-md rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300 flex flex-col sm:flex-row">
                      <img src="https://i.postimg.cc/0jRyzh18/car-rental.jpg" alt="Car Rental" className="w-full sm:w-48 h-48 sm:h-auto object-cover" />
                      <div className="p-6 flex-grow">
                        ...
                        <p className="text-gray-600 mb-4">{rental.address}</p>
                        ...
                        {rental.distance && (
                          <p className="text-gray-800 mb-2 text-lg sm:text-xl">{`Distance: ${rental.distance.toFixed(2)} km`}</p>
                        )}
