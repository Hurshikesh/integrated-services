  ...
  const fetchDoctors = async (userCoordinates) => {
    ...
    try {
      const hereResponse = await fetch(
        `https://discover.search.hereapi.com/v1/discover?at=${userCoordinates.lat},${userCoordinates.lon}&q=hospital&apiKey=smQYaHs6kqHnMongUhEHKnBIXpmilQacnaE9xDCSFYY`
      );
      const hereData = await hereResponse.json();
      console.log('HERE API Response:', hereData);
  
      const backendResponse = await axios.get(`/api/health/hospitals?lon=${userCoordinates.lon}&lat=${userCoordinates.lat}&domain=Healthcare&serviceType=Hospital`);
    ...
    <div className="min-h-screen bg-gray-100">
      <main className="container mx-auto px-4 py-8">
        <section className="mb-12">
          <h2 className="text-2xl md:text-3xl font-bold mb-6 text-center text-blue-600">Search for Hospitals Near You</h2>
                ...
                <div className="space-y-8">
                  {doctors.map((doctor) => (
                    <div key={doctor.id} className="bg-white shadow-md rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300 flex flex-col md:flex-row">
                      <img src="https://i.postimg.cc/XJj1QCLD/fotor-ai-20240709111149.jpg" alt="Doctor" className="w-full md:w-48 h-48 md:h-auto object-cover" />
