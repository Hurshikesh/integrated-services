...
import { faCar, faBicycle, faWalking, faPhone, faClock, faMapMarkerAlt, faMap, faStar, faStarHalfAlt, faHeart } from '@fortawesome/free-solid-svg-icons';
...
const GroceryPage = () => {
  ...
  const [serviceType, setServiceType] = useState('grocery'); // State to store selected service type
  ...
  const fetchServices = async (userCoordinates) => {
    ...
    try {
      const queryMap = {
        grocery: 'grocery%20store',
        supermarket: 'supermarket',
        convenience: 'convenience%20store',
      };
