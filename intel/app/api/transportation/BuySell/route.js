...
export async function GET(request) {
  try {
    ...
    const domain = searchParams.get('domain') || 'Transportation';
    const serviceType = searchParams.get('serviceType') || 'Buy And Sell Cars';
    ...
    const BuySell = await Provider.find({
      domain,
      serviceType,
      location: {
        $near: {
    ...
    return NextResponse.json({ success: true, data: BuySell });
  } catch (error) {
