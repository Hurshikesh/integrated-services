...
export async function GET(request) {
  try {
    ...
    const serviceType = searchParams.get('serviceType') || 'School';
    ...
    const schools = await Provider.find({
      domain,
      serviceType,
      location: {
        $near: {
    ...
    return NextResponse.json({ success: false, error: 'Error fetching schools', details: error.message }, { status: 500 });
  }
}
